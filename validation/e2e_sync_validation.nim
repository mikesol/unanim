## validation/e2e_sync_validation.nim
## E2E sync protocol validation: exercises the full sync protocol against
## real Cloudflare infrastructure — event sync, 409 reconciliation,
## offline queue, persistence, and sequence continuity.
##
## Compile and run: nim c -r validation/e2e_sync_validation.nim
## Outputs:
##   validation/e2e_sync_deploy/worker.js + wrangler.toml
##   validation/e2e_sync_test/index.html
...
const deployDir = "validation/e2e_sync_deploy"
static:
  generateArtifacts("unanim-e2e-sync", deployDir)

# --- Part 2: Browser test page generation ---

# Reuse the already-deployed unanim-todo Worker from #28
...
let testOutputDir = getCurrentDir() / "validation" / "e2e_sync_test"
...
echo ""
echo "Sync validation page generated at: " & testOutputDir & "/index.html"
echo "Open in browser to run E2E sync validation tests."
